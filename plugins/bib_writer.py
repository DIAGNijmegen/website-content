...
class PublicationsGenerator:
    def get_publications_by_author(self, global_index, list_researchers):
    ...
    def generate_md_bibitem(self):
        ...
        list_researchers = self.get_list_people()
        author_index, filtered_publications = self.get_publications_by_author(global_index, list_researchers)
        
        # Writes single md files per publication
        self.write_single_publication_md(global_index, string_rules, filtered_publications, out_dir, json_path)
        ...
        self.write_author_publications_md(global_index, author_index, list_researchers, out_dir, string_rules)
        dict_pubs = self.write_list_publications_md(global_index, filtered_publications, out_dir, string_rules)
    ...
    def append_publication_md(self, global_index, bib_key, html_format, go_parent_dir=False):
        ...
        if 'journal' in bib_item.entry and 'arxiv' in bib_item.entry['journal'].lower():
            ...
            url_arxiv = self.get_arxiv_id_from_title(bib_item.entry['journal'])
            pub_html += ' <a href=\"' + url_arxiv + '/\">arXiv</a>'
            pub_type = '@Preprint'
    ...
    def write_list_publications_md(self, global_index, filtered_publications, out_dir, string_rules):
        html_format = bibtexformatter.HTML_Formatter(string_rules)
        
        dict_pubs = {}
        for bib_key in filtered_publications:
            bib_key = bib_key.lower()
            html_bibkey, year, pub_type, pub_details = self.append_publication_md(global_index, bib_key, html_format,
                                                                                  go_parent_dir=False)
            dict_pubs[bib_key] = {}
    ...
    def write_author_publications_md(self, global_index, author_index, list_researchers, out_dir, string_rules):
        list_bibs_error = []
        html_format = bibtexformatter.HTML_Formatter(string_rules)
        ...
        for researcher_names in list_researchers:
            ...
            self.write_md_pass(out_path, md_format)
    
    def write_single_publication_md(self, global_index, string_rules, filtered_publications, out_dir, json_path):
        html_format = bibtexformatter.HTML_Formatter(string_rules)
        ...
        for bibitem in filtered_publications:
            ...
            if pub_type.lower() == '@phdthesis':
                ...
                if 'url' in global_index[bibitem].entry and 'arxiv' in global_index[bibitem].entry['url']:
                    ...
                    url_pub = global_index[bibitem].entry['url']
                    md_format += 'urlweb: ' + url_pub + '\n'
                if 'journal' in global_index[bibitem].entry and 'arxiv' in global_index[bibitem].entry[
                    'journal'].lower():
                    url_arxiv = self.get_arxiv_id_from_title(global_index[bibitem].entry['journal'])
